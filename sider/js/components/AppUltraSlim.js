...
const AppUltraSlim = {
      ...
      <footer class="app-footer">
        <div class="footer-content">
          <div class="footer-left">
            <div class="credits-section" v-if="isAuthenticated">
          ...
          <div class="footer-right">
            <div class="footer-user-avatar" v-if="isAuthenticated && user">
              <div class="footer-user-initial">{{ user.name ? user.name[0].toUpperCase() : 'U' }}</div>
            </div>
          </div>
        </div>
      </footer>
  ...
  async mounted() {
    ...
    let previousAuthState = JSON.stringify(window.authModule.getAuthState());
    window.authModule.startPeriodicAuthCheck((authState) => {
      const currentAuthState = JSON.stringify(authState);
