...
class ChaturbateClientEventHandler(BaseEventHandler):
    ...
    async def handle_chat(self, event: Message) -> None:
        ...
        try:
            ...
            if self.enable_logging:
                logger.debug(f"💬 Chat: {username}: {message[:100]}{'...' if len(message) > 100 else ''}")
    ...
    async def _is_spam_message(self, message: str) -> bool:
        ...
        spam_indicators = [
            ...
            len(set(message.lower().split())) < len(message.split()) * 0.3,  # Too repetitive
        ]
    ...
    async def handle_private_message(self, event: Message) -> None:
        ...
        try:
            ...
            if self.enable_logging:
                logger.info(f"📧 Private message from '{username}' (length: {len(message)})")
    ...
    async def handle_media_purchase(self, event: Event) -> None:
        ...
        try:
            ...
            if not hasattr(event.object, 'media') or not event.object.media:
                logger.warning(f"Media purchase event from {username} missing media data")
                return
            ...
            if self.enable_logging:
                logger.info(f"🎬 Media purchase: '{media_name}' by {username} for {tokens} tokens")
    ...
    async def _process_media_purchase(self, username: str, media_name: str, tokens: int) -> None:
        ...
        """
        logger.debug(f"Processing media purchase: {media_name} by {username} for {tokens} tokens")
...
def is_private_message(event: Union[Event, Message]) -> bool:
    ...
    try:
        if isinstance(event, Message):
            return (hasattr(event, 'from_user') and event.from_user is not None and
                   hasattr(event, 'to_user') and event.to_user is not None)
        elif hasattr(event, 'object') and hasattr(event.object, 'from_user'):
            return (event.object.from_user is not None and 
                   hasattr(event.object, 'to_user') and event.object.to_user is not None)
        return False
    except Exception:
        return False
