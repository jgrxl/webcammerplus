...
reply_request_model = api.model(
    "ReplyRequest",
    {
        "original_text": fields.String(
            required=True, description="The original message to reply to"
        ),
        "response_idea": fields.String(
            required=True, description="General idea or direction for the response"
        ),
        "style": fields.String(
            required=True,
            description="Writing style (e.g., formal, casual, professional)",
        ),
        "to_lang": fields.String(
            required=True, description="Target language for the reply"
        ),
    },
)

reply_response_model = api.model(
    "ReplyResponse",
    {
...
class Reply(Resource):
    ...
    def post(self):
        ...
        generated_reply = reply_text(
            req.original_text, req.response_idea, req.style, req.to_lang
        )
