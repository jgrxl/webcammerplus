...
from flask import Response, abort, jsonify, request, redirect, session, url_for
...
usage_response_model = api.model('UsageResponse', {
    'tier': fields.String(description='Subscription tier'),
    'tier_display_name': fields.String(description='Human-readable tier name'),
    'subscription_status': fields.String(description='Subscription status'),
    'current_period_start': fields.String(description='Current billing period start'),
...
def setup_auth_routes(app, auth0):
    ...
    def logout():
        ...
        config = Auth0Config()
        logout_url = f"https://{config.domain}/v2/logout?" + \
