...
class Linter:
    ...
    def run(self, cwd: str) -> bool:
        ...
        try:
            ...
            print(f"{Colors.RED}✗ {self.name} not found. Install with: pip install -r requirements-dev.txt{Colors.END}")
            return False
...
def generate_report(results: Dict[str, bool]) -> None:
    ...
    if passed == total:
        ...
        print(f"{Colors.RED}{Colors.BOLD}❌ Some checks failed. Run with --fix to auto-fix formatting issues.{Colors.END}")
        return False
...
def main():
    ...
    if not server_dir.exists():
        print(f"{Colors.RED}Error: server directory not found at {server_dir}{Colors.END}")
        sys.exit(1)
    ...
    if fix_mode:
        print(f"{Colors.YELLOW}Running in fix mode - will auto-format code{Colors.END}\n")
