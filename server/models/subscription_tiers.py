...
def is_usage_within_limits(tier: SubscriptionTier, 
                          translations: int = 0,
                          replies: int = 0, 
                          writes: int = 0,
                          influx_queries: int = 0) -> bool:
    """Check if current usage is within tier limits."""
    limits = get_tier_limits(tier)
    
    # Check each limit (-1 means unlimited)
    if limits.translations_per_month != -1 and translations > limits.translations_per_month:
        return False
    ...
    if limits.writes_per_month != -1 and writes > limits.writes_per_month:
        return False
    if limits.influx_queries_per_month != -1 and influx_queries > limits.influx_queries_per_month:
        return False
