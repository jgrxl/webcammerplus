...
def wait_for_influxdb(url: str, 
    ...
    logger.info(f"Waiting for InfluxDB at {url} to become healthy (timeout: {timeout}s)")
            ...
            if response.status_code == 200:
                try:
                    ...
                    if status == "pass":
                        ...
                        logger.info(f"InfluxDB is healthy after {elapsed:.1f}s ({attempt} attempts)")
                        return True
                    else:
                        logger.debug(f"InfluxDB status: {status} (not yet healthy)")
                        
                except ValueError as e:
                    logger.warning(f"Invalid JSON response from InfluxDB health endpoint: {e}")
    ...
    elapsed = time.time() - start_time
    logger.warning(f"InfluxDB health check timed out after {elapsed:.1f}s ({attempt} attempts)")
    return False
