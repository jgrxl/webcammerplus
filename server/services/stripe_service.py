...
from models.subscription_tiers import SubscriptionTier, SUBSCRIPTION_TIERS, get_tier_by_name
...
class StripeService:
    ...
    def __init__(self):
        ...
        self.price_ids = {
            'pro_monthly': os.getenv('STRIPE_PRICE_PRO_MONTHLY', 'price_pro_monthly_placeholder'),
            'pro_yearly': os.getenv('STRIPE_PRICE_PRO_YEARLY', 'price_pro_yearly_placeholder'),
            'max_monthly': os.getenv('STRIPE_PRICE_MAX_MONTHLY', 'price_max_monthly_placeholder'),
            'max_yearly': os.getenv('STRIPE_PRICE_MAX_YEARLY', 'price_max_yearly_placeholder'),
        }
    
    def create_customer(self, user_email: str, user_name: Optional[str] = None, 
                       auth0_id: Optional[str] = None) -> stripe.Customer:
    ...
    def create_checkout_session(self, 
        ...
        try:
            ...
            if not price_id or price_id.endswith('_placeholder'):
                raise ValueError(f"Price ID not configured for {tier_name} {billing_cycle}")
            ...
            logger.info(f"Created checkout session: {session.id} for {tier_name} {billing_cycle}")
            return session
    ...
    def _process_subscription_event(self, subscription_data: Dict[str, Any], 
                                  event_type: str) -> Optional[Subscription]:
        ...
        try:
            ...
            logger.info(f"Processed subscription event: {event_type} for {subscription_id}")
            return subscription
    ...
    def _process_invoice_event(self, invoice_data: Dict[str, Any], 
                             event_type: str) -> Optional[Subscription]:
        ...
        try:
            ...
            return self._process_subscription_event(subscription, 'subscription.updated')
