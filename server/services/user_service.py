...
from models.subscription_tiers import SubscriptionTier, get_tier_limits, is_usage_within_limits
...
class UserService:
    ...
    def create_or_update_user(self, auth0_profile: Dict[str, Any]) -> User:
        ...
        if existing_user:
            ...
            existing_user.email_verified = auth0_profile.get('email_verified', existing_user.email_verified)
    ...
    def update_user_subscription(self, auth0_id: str, 
    ...
    def check_usage_limits(self, auth0_id: str, 
                          operation: str,
                          increment: int = 1) -> tuple[bool, Optional[str]]:
        ...
        if not is_within_limits:
            ...
            return False, f"Usage limit exceeded. Your {user.subscription_tier.value} plan allows {operation_limit} {operation} per month"
        ...
        return {
            ...
            'usage': {
                'translations': {
                    ...
                    'limit': limits.translations_per_month if limits.translations_per_month != -1 else "unlimited"
                },
                'replies': {
                    'used': user.replies_used,
                    'limit': limits.replies_per_month if limits.replies_per_month != -1 else "unlimited"
                },
                'writes': {
                    'used': user.writes_used,
                    'limit': limits.writes_per_month if limits.writes_per_month != -1 else "unlimited"
                },
                'influx_queries': {
                    'used': user.influx_queries_used,
                    'limit': limits.influx_queries_per_month if limits.influx_queries_per_month != -1 else "unlimited"
                }
            },
            'features': {
    ...
    def get_subscription_by_customer(self, stripe_customer_id: str) -> Optional[Subscription]:
    ...
    def update_subscription_status(self, stripe_subscription_id: str, 
                                 ...
                                 current_period_end: Optional[datetime] = None) -> Optional[Subscription]:
